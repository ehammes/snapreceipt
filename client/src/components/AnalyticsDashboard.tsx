...
const AnalyticsDashboard: React.FC = () => {
  ...
  const handleEmailSummary = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(API_ENDPOINTS.EMAIL_SEND_SUMMARY, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        ...
        <div className="flex flex-col mb-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-4 mt-4 sm:mt-0">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white transition-colors bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {emailSending ? (
                <>
